...
void clearServoReceiveBuffer();


class Servo{
	public:
		Servo(volatile uint8_t* volatile receiveBuffer){
			m_baudrate=57600;
			m_usart=USART2;
			delay_init();
			
			m_receiveBufferStart=receiveBuffer;
			m_receiveBufferEnd=receiveBuffer;
		}
		Servo(volatile uint8_t* volatile receiveBuffer, USART_TypeDef * usart, u32 baudrate=57600){
			m_usart=usart;
			m_baudrate=baudrate;
			delay_init();
	...
	volatile uint8_t* volatile m_receiveBufferStart;
	volatile uint8_t* volatile m_receiveBufferEnd;
};
